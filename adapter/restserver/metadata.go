...
var _parser = map[string]func(string) interface{}{
	"mirror": func(mirrorStr string) interface{} {
		...
		if err != nil {
			log.BaseLogger.Warn(" failed to parse mirror", zap.Error(err), zap.String("mirrorStr", mirrorStr))
			return false
		}
		if !val {
			log.BaseLogger.Warn(" request mirrorStr value is false", zap.String("mirrorStr", mirrorStr))
		}
		return val
	},
