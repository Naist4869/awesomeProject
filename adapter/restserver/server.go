...
import (
	...
	"github.com/Naist4869/awesomeProject/tool/dsn"
...
func addFlag(fs *flag.FlagSet) {
	v := os.Getenv("HTTP")
	if v == "" {
		v = "tcp://0.0.0.0:8000/?timeout=5s&readTimeout=5s&writeTimeout=5s"
	}
	fs.StringVar(&httpDSN, "http", v, "listen http dsn, or use HTTP env variable.")
}
...
func parseDSN(rawdsn string) *ServerConfig {
	...
	if err != nil {
		panic(errors.Wrapf(err, "invalid dsn: %s", rawdsn))
	}
...
func (engine *Engine) Start() error {
	...
	if err != nil {
		return errors.Wrapf(err, "listen tcp: %s", conf.Addr)
	}

	log.BaseLogger.Info("start http listen", zap.String("addr: ", l.Addr().String()))
	server := &http.Server{
	...
	go func() {
		if err := engine.RunServer(server, l); err != nil {
			if errors.Cause(err) == http.ErrServerClosed {
				log.BaseLogger.Info("server closed")
				return
			}
			panic(errors.Wrapf(err, "engine.ListenServer(%+v, %+v)", server, l))
		}
	}()
...
func NewEngine(conf *ServerConfig) *Engine {
	if conf == nil {
		if !flag.Parsed() {
...
func DefaultServer(conf *ServerConfig) *Engine {
	...
	engine.Use(Recovery(), Logger(log.BaseLogger))
	return engine
}
...
func (engine *Engine) metadata() HandlerFunc {
	return func(c *Context) {
		c.JSON(http.StatusOK, engine.metastore)
	}
}
